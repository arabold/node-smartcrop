...
var ccv = {
	...
	detect_objects : function (image, interval, min_neighbors) {
		var params = {
			interval: interval,
			min_neighbors: min_neighbors
		};
		params.canvas = image.clone().greyscale();
		params.cascade = cascade;
		params.scale = Math.pow(2, 1 / (params.interval + 1));
		params.next = params.interval + 1;
		params.scale_upto = Math.floor(Math.log(Math.min(
				params.canvas.bitmap.width / params.cascade.width, 
				params.canvas.bitmap.height / params.cascade.height)) / Math.log(params.scale));

		for (var i = 0; i < params.cascade.stage_classifier.length; i++) {
...
//						"data" : pyr[i * 4].getContext("2d").getImageData(0, 0, pyr[i * 4].width, pyr[i * 4].height).data };
				pyr[i * 4] = pyr[0].clone().resize(
						pyr[0].bitmap.width / Math.pow(scale, i), 
						pyr[0].bitmap.height / Math.pow(scale, i));
				ret[i * 4] = pyr[i * 4].bitmap;
			}
...
//						"height" : pyr[i * 4].height,
//						"data" : pyr[i * 4].getContext("2d").getImageData(0, 0, pyr[i * 4].width, pyr[i * 4].height).data };
				pyr[i * 4] = pyr[i * 4 - next * 4].clone().resize(
						Math.floor(pyr[i * 4 - next * 4].bitmap.width / 2), 
						Math.floor(pyr[i * 4 - next * 4].bitmap.height / 2));
				ret[i * 4] = pyr[i * 4].bitmap;
			}
...
//						"data" : pyr[i * 4 + 1].getContext("2d").getImageData(0, 0, pyr[i * 4 + 1].width, pyr[i * 4 + 1].height).data };
				pyr[i * 4 + 1] = pyr[i * 4 - next * 4].clone().resize(
						Math.floor(pyr[i * 4 - next * 4].bitmap.width / 2), 
						Math.floor(pyr[i * 4 - next * 4].bitmap.height / 2));
				ret[i * 4 + 1] = pyr[i * 4 + 1].bitmap;
...
//						"data" : pyr[i * 4 + 2].getContext("2d").getImageData(0, 0, pyr[i * 4 + 2].width, pyr[i * 4 + 2].height).data };
				pyr[i * 4 + 2] = pyr[i * 4 - next * 4].clone().resize(
						Math.floor(pyr[i * 4 - next * 4].bitmap.width / 2), 
						Math.floor(pyr[i * 4 - next * 4].bitmap.height / 2));
				ret[i * 4 + 2] = pyr[i * 4 + 2].bitmap;
...
//						"data" : pyr[i * 4 + 3].getContext("2d").getImageData(0, 0, pyr[i * 4 + 3].width, pyr[i * 4 + 3].height).data };
				pyr[i * 4 + 3] = pyr[i * 4 - next * 4].clone().resize(
						Math.floor(pyr[i * 4 - next * 4].bitmap.width / 2), 
						Math.floor(pyr[i * 4 - next * 4].bitmap.height / 2));
				ret[i * 4 + 3] = pyr[i * 4 + 3].bitmap;
			}
			return ret;
		};
		...
		var result = post([ core(pre(1),0,0) ]);
